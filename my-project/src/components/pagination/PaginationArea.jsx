...
const PaginationArea = ({pageNumber, paginate, pageStart,next,prev}) => {
  ...
  <ul class="inline-flex -space-x-px text-sm">
    ...
    <li onClick={()=>paginate(item)}>
      <a class={pageStart == i + 1 ? "flex items-center justify-center px-3 h-8 leading-tight text-white bg-black border border-gray-300" : "flex items-center justify-center px-3 h-8 leading-tight text-[#000000] bg-[white] border border-gray-300"} >{item + 1}</a>
    </li>
     ))}
